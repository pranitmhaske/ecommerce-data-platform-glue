...
This architecture is processing e-commerce datasets (events, transactions, users) with clear seperation of responsibility of ingestion, transformation, modeling.
...
- Amazon Redshift Serverless stores data for analyst and bi tools to query.
...
**Characteristics**
- Stores 9.1 GB messy source data
...
This is created by event-driven Airflow ingestion DAGs.
...
**Purpose**  
Ensure ingestion integrity and isolate bad files early and allowing valid data to move forward for processing.

---

### Layer 3 — SILVER Zone (Amazon S3)

This is created by AWS Glue Spark job: **Bronze → Silver**
...
- SCD-style user history handling but with no tie breaker(in real systems, if same situation happens at least ingestion timestamp is must)
- Data quality validation
- Event_date partition enforcement
- Data quality metrics written to S3 for observability so issues are visible rather than hidden

**Purpose**  
Create clean, trustworthy, analytics-ready datasets.

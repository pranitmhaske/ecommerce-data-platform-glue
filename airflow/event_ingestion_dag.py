...
with DAG(
    dag_id="raw_to_bronze_events_event_driven",
    start_date=datetime(2025, 1, 1),
    schedule_interval=None,          # event-driven
    catchup=False,
    max_active_runs=1,
    default_args=default_args,
    description="Event-driven RAW → BRONZE for events",
):

    # SENSOR: waits for ANY file in RAW/events/*
    wait_for_raw_file = S3KeySensor(
        task_id="wait_for_raw_event_file",
